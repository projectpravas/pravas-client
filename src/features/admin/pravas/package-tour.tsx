...
const PackageForm: React.FunctionComponent<IPackageFormProps> = (props) => {
  ...
  const setPlanDay = () => {
    const tourData = { ...tour };
    const tourPlanData = { ...tourData?.tourPlan };

    const itineraryArr: any = [tourPlanData?.itinerary];

    daysArray.forEach((v, i) => {
      const day = itineraryArr[i];
      if (day) day.day = Number(i) + 1;
    });

    setTour({
      ...tour,
      tourPlan: { ...tour.tourPlan, itinerary: itineraryArr },
    });
  };

  const daysCount = (day: number) => {
    const arr = [];
    const itineraryDataArr = [];

    for (let i = 0; i < day; i++) {
      arr.push(i + 1);
      const title = `planTitle/${i + 1}`;
      const desc = `planDesc/${i + 1}`;
      itineraryDataArr.push({
        day: i + 1,
        [title]: "",
        [desc]: "",
        meals: { breakfast: false, lunch: false, dinner: false },
    ...
    setDaysArray(arr);
    // set itinerary data & Duration- day and nights
    setTour({
      ...tour,
      duration: { days: day, nights: day - 1 },
      tourPlan: { ...tour?.tourPlan, itinerary: itineraryDataArr },
    });
  };
  ...
  const handleTourDuration = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value, checked } = e?.target;
  ...
  const handleTour = (e: React.ChangeEvent<HTMLInputElement>, day = NaN) => {
    const { name, value, checked } = e?.target;
    ...
    if (day) {
      if (name == "breakfast") {
        itineraryArr[day - 1].meals.breakfast = checked;
      } else if (name == "lunch") {
        itineraryArr[day - 1].meals.lunch = checked;
      } else if (name == "dinner") {
        itineraryArr[day - 1].meals.dinner = checked;
      }
      ...
      if (name?.includes("planTitle") || name?.includes("planDesc")) {
        const index = itineraryArr?.findIndex((obj: any) =>
          obj?.hasOwnProperty(name)
        );

        if (index > -1)
          itineraryArr.splice(index, 1, {
            ...itineraryArr[index],
            [name]: value,
          });
